...
MOCK_MODE = False          # Set False to use Muse / any LSL EEG stream
...
class StressManager:
    """
    Dual-input stress provider with a 20-second baseline calibration phase.

    Calibration flow
    ----------------
    1. Immediately after construction, is_calibrated = False.
    2. Call update() every frame during the CalibrationState; raw EEG (or a
       mock ramp) is appended to baseline_buffer.
    3. Once CALIBRATION_DURATION seconds have elapsed, _finalise_calibration()
       computes baseline_value = mean(baseline_buffer) and sets is_calibrated.
    4. During gameplay, stress_level is derived as:
           stress = clamp((raw - baseline) / SENSITIVITY, 0, 1)
       so the player always starts at ~0 regardless of their resting EEG.

    Mock calibration
    ----------------
    In MOCK_MODE the baseline is simulated with a gentle sine wave so the
    progress bar fills naturally without real hardware.

    Public API
    ----------
    update(keys, dt)  → call every frame; drives calibration then stress
    is_calibrated     → bool
    calibration_progress → float 0‥1 (for the loading bar)
    stress_level      → float 0‥1  (only meaningful after calibration)
    baseline_value    → float (mean raw amplitude at rest)
    """

    CALIBRATION_DURATION = 20.0   # seconds
    MOCK_DELTA           = 0.008  # stress change per key-press frame
    # How many µV above baseline = full stress (tune per subject)
    SENSITIVITY          = 350.0
    ...
    def __init__(self, mock_mode: bool = True):
        ...
        self.calibration_progress   = 0.0   # 0.0 → 1.0
        self._calib_timer           = 0.0
        self.baseline_buffer: list  = []    # raw amplitude samples
    ...
    def _update_calibration(self, dt: float):
        ...
        self.calibration_progress    = min(1.0,
            self._calib_timer / self.CALIBRATION_DURATION)

        # Collect a raw sample each frame
    ...
    def _read_raw_amplitude(self) -> float | None:
        """
        Returns a single raw amplitude value for the current frame.
        In MOCK_MODE we return a gentle synthetic sine so the buffer is
        populated with plausible resting-state data.
        """
        if self.mock_mode:
            # Simulate calm resting EEG: ~200 µV base + small oscillation
            t = self._calib_timer
            return 200.0 + 30.0 * math.sin(t * 1.1) + random.gauss(0, 15)

        # Real EEG path: read latest sample from ring buffer
        if len(self._eeg_buffer) < 1:
            return None
    ...
    def _lsl_reader(self):
        """Background thread: continuously drains the LSL inlet."""
    ...
    def _update_eeg(self):
        """
        Dynamic stress = clamp((current_raw - baseline) / SENSITIVITY, 0, 1).

        Because baseline_value was recorded at rest, a player sitting still
        will always start near 0.0 regardless of their absolute EEG amplitude.

        ASSET: Swap the amplitude heuristic below for a proper FFT-derived
               Alpha/Beta ratio using alpha_beta_ratio_to_stress() once you
               have stable spectral data.
        """
        ...
        # Baseline-relative stress
        raw_stress = (mean_amp - self.baseline_value) / self.SENSITIVITY

        # Exponential moving average smoothing
        ema_alpha = 0.05
        self.stress_level = max(0.0, min(1.0,
            ema_alpha * raw_stress + (1 - ema_alpha) * self.stress_level
        ))
...
class AudioManager:
    ...
    def __init__(self):
        ...
        #   "sting_med"    : pygame.mixer.Sound("assets/audio/know_youre_in.wav"), (https://www.youtube.com/watch?v=MtGyTnkvNKk)
        #   "sting_high"   : pygame.mixer.Sound("assets/audio/i_see_you.wav"),
        #   "crash_sound"  : pygame.mixer.Sound("assets/audio/crash.wav"),
        #   "ambient_wind" : pygame.mixer.Sound("assets/audio/wind_ambient.wav"),
    ...
    def play_ambient(self):
        """Start the barely-audible calibration wind loop (does nothing if
        already playing)."""
    ...
    def stop_ambient(self):
        """Fade out the calibration ambient before gameplay music starts."""
        self._ch_ambient.fadeout(1500)
        self._ambient_playing = False

    def fade_in_music(self, stress: float = 0.0):
        """
        Called once when gameplay begins.  Starts the correct music track
        with a 3-second fade so it feels like a reward after calibration.
        """
    ...
    def _update_music(self, stress: float):
        if stress < STRESS_MED:                       # New code
            key = "calm_music"
        elif stress < STRESS_HIGH:
            key = "tense_music"
        else:
            key = "tense_music"                       # New code
        
        #key = "calm_music" if stress < STRESS_HIGH else "tense_music" #old code
...
class Player:
    ...
    def draw(self, surface: pygame.Surface, cam_offset: int):
        ...
        # Eyes
        eye_y = r.top + 10
        pygame.draw.circle(surface, C_WHITE, (r.centerx - 5, eye_y), 3)
        pygame.draw.circle(surface, C_WHITE, (r.centerx + 5, eye_y), 3)
...
class CalibrationState(State):
    """
    "Neural Syncing" screen shown before gameplay.

    What it does
    ------------
    - Starts the quiet ambient wind so there is *something* audible.
    - Counts down 20 seconds while StressManager fills its baseline_buffer.
    - Draws a progress bar, countdown timer, and breathing instruction.
    - Transitions automatically to "gameplay" once is_calibrated == True.

    Visual language: deep blue-black, soft pulsing circle, clinical font —
    contrasts sharply with the red horror of gameplay, making the transition
    feel rewarding.
    """

    # Colours used only in this screen
    _C_BG        = (4,   8,  20)
    _C_RING      = (20,  60, 120)
    _C_RING_FILL = (40, 140, 220)
    _C_BAR_BG    = (20,  30,  50)
    ...
    _DURATION    = StressManager.CALIBRATION_DURATION

    def on_enter(self, game):
        game.audio.play_ambient()
        self._pulse_t    = 0.0
        self._fade_alpha = 255          # start with a black fade-in
        self._done       = False

        # Pre-bake fonts (avoid re-creating every frame)
    ...
    def on_exit(self, game):
        ...
        # Reward: fade game music in as gameplay begins
        game.audio.fade_in_music(stress=0.0)

    def handle_event(self, game, event):
        # Allow skipping calibration in mock mode (useful for rapid iteration)
        if (game.stress_mgr.mock_mode and
    ...
    def update(self, game, dt: float):
        ...
        # StressManager drives its own calibration timer via update()
        # (called in Game.run before state.update)
    ...
    def draw(self, game, surface: pygame.Surface):
        ...
        elapsed  = game.stress_mgr._calib_timer
        ...
        # ── Pulsing outer ring ────────────────────────────────────────────────
        pulse  = (math.sin(self._pulse_t * 1.4) + 1) / 2    # 0..1
        radius = int(110 + pulse * 18)
        pygame.draw.circle(surface, self._C_RING, (cx, cy - 60), radius, 3)

        # Arc that fills clockwise with progress (approximated with segments)
        arc_rect = pygame.Rect(cx - radius, cy - 60 - radius, radius * 2, radius * 2)
        start_a  = -math.pi / 2                              # top
        end_a    = start_a + 2 * math.pi * progress
        if progress > 0.01:
            # Draw arc as polyline of short segments
            steps  = max(4, int(progress * 60))
            pts    = []
            for i in range(steps + 1):
                a = start_a + (end_a - start_a) * i / steps
                pts.append((cx + radius * math.cos(a),
                             cy - 60 + radius * math.sin(a)))
            if len(pts) > 1:
                pygame.draw.lines(surface, self._C_RING_FILL, False, pts, 4)

        # ── Countdown timer inside the ring ───────────────────────────────────
        timer_surf = self._f_timer.render(f"{int(remaining) + 1:02d}", True,
                                          self._C_TIMER)
        surface.blit(timer_surf, timer_surf.get_rect(center=(cx, cy - 60)))

        # ── Title ─────────────────────────────────────────────────────────────
        title = self._f_title.render("◈  NEURAL SYNCING  ◈", True, self._C_TITLE)
        surface.blit(title, title.get_rect(center=(cx, cy + 80)))

        # ── Instruction ───────────────────────────────────────────────────────
        lines = [
            "Please remain still and breathe deeply",
            "to calibrate the neural interface.",
        ]
        for i, line in enumerate(lines):
            s = self._f_body.render(line, True, self._C_BODY)
        ...
        pygame.draw.rect(surface, self._C_BAR_BG,
                         (bar_x - 2, bar_y - 2, bar_w + 4, bar_h + 4),
                         border_radius=10)
        filled = int(bar_w * progress)
        if filled > 0:
            pygame.draw.rect(surface, self._C_BAR_FILL,
                             (bar_x, bar_y, filled, bar_h),
                             border_radius=8)
        pygame.draw.rect(surface, self._C_RING_FILL,
                         (bar_x - 2, bar_y - 2, bar_w + 4, bar_h + 4),
                         2, border_radius=10)

        pct_label = self._f_small.render(
            f"Signal Loading …  {int(progress * 100):3d}%", True, self._C_BODY)
        surface.blit(pct_label, pct_label.get_rect(center=(cx, bar_y + bar_h + 16)))

        # Mock-mode shortcut hint
        if game.stress_mgr.mock_mode:
            hint = self._f_small.render(
                "MOCK MODE — press ENTER to skip calibration", True, (60, 80, 110))
            surface.blit(hint, hint.get_rect(center=(cx, SCREEN_H - 28)))
...
class MenuState(State):
    ...
    def handle_event(self, game, event):
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_RETURN:
                game.change_state("calibration")   # → calibration first
    ...
    def draw(self, game, surface):
        ...
        hint = font_hint.render("ENTER — Begin Calibration    ESC — Quit", True, C_HUD_TEXT)
...
class Game:
    ...
    def change_state(self, name: str):
        ...
        # Re-instantiate transient states so they always start fresh
        if name == "calibration":
            # Also reset the StressManager so baseline is collected anew
            self.stress_mgr = StressManager(mock_mode=MOCK_MODE)
            self._states["calibration"] = CalibrationState()
            self._current_state = self._states["calibration"]
        if name == "gameplay":
